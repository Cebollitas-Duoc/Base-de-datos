...
CREATE SEQUENCE seq_servicio_id_servicioextra START WITH 1 NOCACHE ORDER;

CREATE OR REPLACE TRIGGER trg_servicio_id_servicioextra BEFORE
    INSERT ON t_servicioextra
    FOR EACH ROW
    WHEN ( new.id_servicioextra IS NULL )
BEGIN
    :new.id_servicioextra := seq_servicio_id_servicioextra.nextval;
END;
/
