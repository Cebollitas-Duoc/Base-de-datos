...
CREATE TABLE t_conector_servicioextra_reserva (
    ...
    id_departamento   NUMBER NOT NULL,
    id_servicioextra  NUMBER NOT NULL,
    id_servicioextra2 NUMBER NOT NULL
);
...
--  ERROR: FK name length exceeds maximum allowed length(30) 
ALTER TABLE t_conector_servicioextra_reserva
    ADD CONSTRAINT t_conector_servicioextra_reserva_t_conector_departamento_servicioextra_fk FOREIGN KEY ( id_servicioextra2 )
...
ALTER TABLE t_reserva
    ADD CONSTRAINT t_reserva_t_reporte_fkv1 FOREIGN KEY ( id_reporte1 )
        REFERENCES t_reporte ( id_reporte );
...
CREATE SEQUENCE seq_foto_id_foto START WITH 1 NOCACHE ORDER;

CREATE OR REPLACE TRIGGER trg_foto_id_foto BEFORE
    INSERT ON t_foto
    FOR EACH ROW
    WHEN ( new.id_foto IS NULL )
BEGIN
    :new.id_foto := seq_foto_id_foto.nextval;
END;
/
